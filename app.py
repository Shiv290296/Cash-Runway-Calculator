...
cash_balance = st.sidebar.number_input(
    "Current Cash Balance ($)", min_value=0, value=2_000_000, step=50_000, format="%d"
)

st.sidebar.subheader("Monthly Revenue")
monthly_revenue = st.sidebar.number_input(
    "Current Monthly Revenue ($)", min_value=0, value=150_000, step=10_000, format="%d"
)
...
cogs = st.sidebar.number_input("COGS ($)", min_value=0, value=30_000, step=5_000, format="%d")
...
accounts_receivable = st.sidebar.number_input("Accounts Receivable ($)", min_value=0, value=120_000, step=10_000, format="%d")
accounts_payable = st.sidebar.number_input("Accounts Payable ($)", min_value=0, value=80_000, step=10_000, format="%d")
other_assets = st.sidebar.number_input("Other Assets ($)", min_value=0, value=50_000, step=10_000, format="%d")
other_liabilities = st.sidebar.number_input("Other Liabilities ($)", min_value=0, value=30_000, step=10_000, format="%d")
...
def calculate_runway(cash, revenue, rev_growth, expenses, exp_growth, months):
    ...
    running_cash = cash
    monthly_rev = revenue
    monthly_exp = expenses
    for month in range(1, months + 1):
        net_burn = monthly_exp - monthly_rev
        running_cash = running_cash - net_burn
        data.append({
            "Month": month,
            "Revenue": round(monthly_rev),
            "Expenses": round(monthly_exp),
            "Net Burn": round(net_burn),
            "Cash Balance": round(running_cash),
        })
        if running_cash <= 0:
            break
        monthly_rev = monthly_rev * (1 + rev_growth / 100)
        monthly_exp = monthly_exp * (1 + exp_growth / 100)
    return pd.DataFrame(data)

def calculate_full_projection(cash, revenue, rev_growth, expenses, exp_growth, months,
                               customers, churn, cust_growth, cogs_val, ar, ap, o_assets, o_liab):
    data = []
    running_cash = cash
    monthly_rev = revenue
    monthly_exp = expenses
    monthly_cogs = cogs_val
    ...
    for month in range(1, months + 1):
        ...
        data.append({
            ...
            "New Customers": new_customers, "Churned": churned_customers,
            "ARPU": round(arpu), "CAC": round(cac),
            "LTV": round(ltv), "LTV/CAC": round(ltv_cac, 1),
            "NRR (%)": round(nrr, 1), "Churn Rate (%)": round(churn, 1),
            "Revenue": round(monthly_rev), "COGS": round(monthly_cogs),
            "Gross Profit": round(gross_profit), "Gross Margin (%)": round(gross_margin, 1),
            "OPEX": round(total_opex), "EBITDA": round(ebitda), "Net Income": round(net_income),
            "Gross Burn": round(gross_burn), "Net Burn": round(net_burn),
            "Cash Balance": round(running_cash),
            "Runway (Months)": round(months_remaining, 1) if months_remaining < 999 else "N/A",
            "Cash": round(running_cash), "Accounts Receivable": round(monthly_ar),
            "Other Assets": round(o_assets), "Total Assets": round(total_assets),
            "Accounts Payable": round(monthly_ap), "Other Liabilities": round(o_liab),
            "Total Liabilities": round(total_liabilities), "Equity": round(equity),
        })
        ...
        monthly_rev = monthly_rev * (1 + rev_growth / 100)
        monthly_exp = monthly_exp * (1 + exp_growth / 100)
        monthly_cogs = monthly_cogs * (1 + exp_growth / 100)
        monthly_ar = monthly_ar * (1 + rev_growth / 200)
        monthly_ap = monthly_ap * (1 + exp_growth / 200)
...
full_df = calculate_full_projection(
    cash_balance, monthly_revenue, revenue_growth_rate, total_monthly_expenses, expense_growth_rate, forecast_months,
    customer_count, monthly_churn_rate, customer_growth_rate, cogs, accounts_receivable, accounts_payable, other_assets, other_liabilities
)

def get_runway_months(df):
    negative = df[df["Cash Balance"] <= 0]
    if len(negative) > 0:
        return negative.iloc[0]["Month"]
...
with col1:
    st.metric("Current Cash", f"${cash_balance:,.0f}")
with col2:
    st.metric("Gross Burn", f"${gross_burn:,.0f}/mo")
with col3:
    st.metric("Net Burn", f"${net_burn:,.0f}/mo")
with col4:
    if net_burn > 0:
        st.metric("Simple Runway", f"{cash_balance / net_burn:.1f} months")
    else:
        st.metric("Simple Runway", "Cash flow positive")
...
if net_burn <= 0:
    status_text = "Cash Flow Positive"
...
with col1:
    ...
    st.markdown(f"""<div class="{color}">
        <h3>Worst Case</h3>
        <h2>{label}</h2>
        <p style="font-size:13px;line-height:1.6;">What if revenue contracts and operating costs escalate? This scenario models a downside with revenue at ${worst_rev_start:,.0f}/mo ({worst_revenue_multiplier:.0%} of current), OPEX at ${worst_exp_start:,.0f}/mo ({worst_expense_multiplier:.0%} of current), decelerating MoM revenue growth, and accelerating cost inflation. Think churn spike, failed product launch, or key customer loss.</p>
    </div>""", unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown(f"""<div class="{color}">
        <h3>Baseline</h3>
        <h2>{label}</h2>
        <p style="font-size:13px;line-height:1.6;">Your current burn trajectory held constant. Revenue at ${monthly_revenue:,.0f}/mo with {revenue_growth_rate}% MoM growth, total OPEX at ${total_monthly_expenses:,.0f}/mo with {expense_growth_rate}% MoM increase. This assumes no fundraise, no major pivots, and steady unit economics. Your default path if nothing changes.</p>
    </div>""", unsafe_allow_html=True)
...
with chart_tab1:
    ...
    fig_burn.update_layout(
        xaxis_title="Month", yaxis_title="Cash Balance ($)", yaxis_tickformat="$,.0f",
...
with chart_tab2:
    ...
    fig_fan.add_trace(go.Scatter(
        x=worst_df["Month"][:max_len], y=worst_df["Cash Balance"][:max_len],
        name="Worst Case Bound", line=dict(color="rgba(248,113,113,0.3)"), showlegend=False
    ))
    fig_fan.add_trace(go.Scatter(
        x=best_df["Month"][:max_len], y=best_df["Cash Balance"][:max_len],
        name="Confidence Range", fill="tonexty", fillcolor="rgba(74,222,128,0.08)",
        line=dict(color="rgba(74,222,128,0.3)"), showlegend=True
    ))
...
with chart_tab3:
    w_labels = ["Starting Cash"]
    w_values = [cash_balance]
    w_measures = ["absolute"]
    ...
    fig_water = go.Figure(go.Waterfall(
        x=w_labels, y=w_values, measure=w_measures,
    ...
    fig_water.update_layout(
        xaxis_title="", yaxis_title="Cash ($)", yaxis_tickformat="$,.0f",
        template="plotly_dark", height=450,
        margin=dict(l=20, r=20, t=40, b=20),
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)"
    )
...
# REVENUE VS EXPENSES
st.header("Revenue vs Expenses (Baseline)")
...
for i in range(len(base_df) - 1):
    row = base_df.iloc[i]
    next_row = base_df.iloc[i + 1]
    months_x = [row["Month"], next_row["Month"], next_row["Month"], row["Month"]]
    if row["Revenue"] >= row["Expenses"]:
        fig_dual.add_trace(go.Scatter(
            x=months_x, y=[row["Revenue"], next_row["Revenue"], next_row["Expenses"], row["Expenses"]],
            fill="toself", fillcolor="rgba(74,222,128,0.15)", line=dict(width=0), showlegend=False, hoverinfo="skip"
        ))
    else:
        fig_dual.add_trace(go.Scatter(
            x=months_x, y=[row["Expenses"], next_row["Expenses"], next_row["Revenue"], row["Revenue"]],
            fill="toself", fillcolor="rgba(248,113,113,0.15)", line=dict(width=0), showlegend=False, hoverinfo="skip"
        ))

fig_dual.update_layout(
    xaxis_title="Month", yaxis_title="Amount ($)", yaxis_tickformat="$,.0f",
...
expense_data = {
    "Category": ["Payroll", "Rent & Infrastructure", "Software & Tools", "Marketing & Sales", "COGS", "Other"],
    "Amount": [payroll, rent_infra, software_tools, marketing, cogs, other_expenses]
}
fig_pie = go.Figure(data=[go.Pie(
    labels=pd.DataFrame(expense_data)["Category"], values=pd.DataFrame(expense_data)["Amount"],
    hole=0.4, marker_colors=["#4ade80", "#3b82f6", "#fbbf24", "#f87171", "#a78bfa", "#6b7280"]
)])
...
# MONTHLY PROJECTIONS (4 Tabs)
st.header("Monthly Projections (Baseline)")
...
with tab_saas:
    ...
    st.dataframe(full_df[saas_cols].style.format({
        "MRR": "${:,.0f}", "ARR": "${:,.0f}", "ARPU": "${:,.0f}",
        "CAC": "${:,.0f}", "LTV": "${:,.0f}", "LTV/CAC": "{:.1f}",
        "NRR (%)": "{:.1f}%", "Churn Rate (%)": "{:.1f}%"
    }), use_container_width=True, hide_index=True)

with tab_pl:
    pl_cols = ["Month", "Revenue", "COGS", "Gross Profit", "Gross Margin (%)", "OPEX", "EBITDA", "Net Income"]
    st.dataframe(full_df[pl_cols].style.format({
        "Revenue": "${:,.0f}", "COGS": "${:,.0f}", "Gross Profit": "${:,.0f}",
        "Gross Margin (%)": "{:.1f}%", "OPEX": "${:,.0f}", "EBITDA": "${:,.0f}", "Net Income": "${:,.0f}"
    }), use_container_width=True, hide_index=True)

with tab_burn:
    burn_cols = ["Month", "Gross Burn", "Net Burn", "Cash Balance", "Runway (Months)"]
    st.dataframe(full_df[burn_cols].style.format({
        "Gross Burn": "${:,.0f}", "Net Burn": "${:,.0f}", "Cash Balance": "${:,.0f}",
    }), use_container_width=True, hide_index=True)
...
with tab_bs:
    ...
    st.dataframe(full_df[bs_cols].style.format({
        "Cash": "${:,.0f}", "Accounts Receivable": "${:,.0f}", "Other Assets": "${:,.0f}",
        "Total Assets": "${:,.0f}", "Accounts Payable": "${:,.0f}",
        "Other Liabilities": "${:,.0f}", "Total Liabilities": "${:,.0f}", "Equity": "${:,.0f}"
    }), use_container_width=True, hide_index=True)
...
if net_burn <= 0:
    ...
    fundraise_needed = net_burn * target_runway - cash_balance
    if fundraise_needed < 0:
        fundraise_needed = 0
    ...
    for raise_amt in [1_000_000, 2_000_000, 3_000_000, 5_000_000, 8_000_000, 10_000_000]:
        ...
        scenarios.append({
            "Raise Amount": f"${raise_amt:,.0f}",
            "Post-Raise Cash": f"${new_cash:,.0f}",
            "Resulting Runway": f"{resulting_runway:.1f} months",
            "Meets Target": "✅" if resulting_runway >= target_runway else "❌"
        })
